...
test('V96: neither flavor mentions tags, cards or the landing page any more', () => {
  for (const flavor of ['greenfield', 'scan-existing']) {
    const d = fs.mkdtempSync(path.join(os.tmpdir(), 'bp-'));
    writeBootstrapPrompt(d, 'demo', flavor);
    const txt = fs.readFileSync(path.join(d, '.claude-bootstrap.txt'), 'utf8');
    assert.doesNotMatch(txt, /tags|card|landing|chip/i, flavor);
    assert.match(txt, /H1/);
  }
});
