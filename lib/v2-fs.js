...
function makeFs({ projectsRoot }) {
  ...
  // Which managed project (first path segment with a sentinel) a path is in.
  function projectOf(rel) {
    const first = rel.split('/')[0];
    if (!first) return null;
    return fs.existsSync(path.join(ROOT, first, '.project-meta.json')) ? first : null;
  }

  return { resolve, gitRoot, list, stat, readText, writeText, mkdir, createFile, rename, diff, log, showAt, projectOf, relToRoot };
}
