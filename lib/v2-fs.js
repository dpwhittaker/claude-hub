...
function makeFs({ projectsRoot }) {
  ...
  // The checked-out branch of the repo a path is in (detached → the short
  // sha in parentheses), or null outside any repo (V101).
  function branchOf(absOrRel) {
    const abs = path.isAbsolute(absOrRel) ? absOrRel : resolve(absOrRel).abs;
    const repo = gitRoot(abs);
    if (!repo) return null;
    try { return git(repo, ['symbolic-ref', '--short', '-q', 'HEAD']).trim() || null; }
    catch {
      try { return '(' + git(repo, ['rev-parse', '--short', 'HEAD']).trim() + ')'; } catch { return null; }
    }
  }
  function branch(raw) {
    const { rel, abs } = resolve(raw);
    const repo = gitRoot(abs);
    return { path: rel, repo: repo ? relToRoot(repo) : null, branch: repo ? branchOf(abs) : null };
  }
  ...
  function list(raw) {
    ...
    return { path: rel, repo: repo ? relToRoot(repo) : null, branch: repo ? branchOf(abs) : null, entries };
  }
  ...
  function stat(raw) {
    ...
    const kind = s.isDirectory() ? 'dir' : (s.isFile() ? 'file' : 'other');
    const repo = gitRoot(abs);
    const out = { path: rel, name: path.basename(abs) || '', kind, size: s.size, mtime: s.mtimeMs, repo: repo ? relToRoot(repo) : null, branch: repo ? branchOf(abs) : null };
    if (kind === 'file') {
  ...
  return { resolve, gitRoot, branch, branchOf, list, stat, readText, writeText, mkdir, createFile, rename, remove, diff, log, showAt, projectOf, relToRoot };
}
