...
function makeSessionStore({ dir, projectsRoot }) {
  ...
  function decorate(s) {
    return { ...s, kind: 'hub', termKey: termKey(s.id), termUrl: termUrl(s.id) };
  }
  ...
  function create({ cwd, agent, profile, title, prompt } = {}) {
    ...
    const s = {
      id, cwd: rel, agent: a, uuid: crypto.randomUUID(), profile: profile || null,
      title: title || null, createdAt: new Date().toISOString(),
    };
    write(s);
  ...
  function update(id, patch = {}) {
    ...
    const { kind: _k, termKey: _tk, termUrl: _tu, ...disk } = s;
    write(disk);
...
module.exports = { makeSessionStore, ID_RE, AGENTS, TERM_UNIT_KEY, termKey, termUrl, newId };
