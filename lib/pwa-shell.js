 ...
 * via the web manifest, a FAB that cycles TERM -> OPEN -> VIEW in single-pane
 * mode and swaps the right half while split, a long-press menu carrying
 * refresh plus the sticky split preference, and keyboard-fit handling so the
 * terminal stays visible when the on-screen keyboard opens.
 *
 * Pure: everything it needs arrives as arguments.
 */

const { escapeHtml } = require('./escape-html');
const { makeSplitLayout } = require('./split-layout');
const { TERM_AGENTS, openAgentMenu } = require('./term-agents');
...
<style>
  ...
  /* Agent badge — only on tabs running something other than the default, so
     a badge always means "not claude". */
  .term-tab .agent-tag {
    font-size: 0.6rem; text-transform: uppercase; letter-spacing: 0.04em;
    padding: 1px 4px; border-radius: 3px; color: inherit;
    border: 1px solid var(--fab-edge); line-height: 1.3;
  }
  .term-tab.active .agent-tag { border-color: var(--accent); }
  /* The "+" menu. position:fixed — the strip scrolls horizontally, so a menu
     positioned inside it would drift off its own button. */
  .term-agent-menu {
    position: fixed; z-index: 60; min-width: 144px;
    background: #131b2c; border: 1px solid var(--fab-edge); border-radius: 8px;
    box-shadow: 0 10px 28px rgba(0,0,0,0.5); padding: 4px; display: flex;
    flex-direction: column;
  }
  .term-agent-item {
    border: none; background: transparent; color: var(--fg); text-align: left;
    font: inherit; font-size: 0.9rem; padding: 10px 12px; border-radius: 6px;
    cursor: pointer;
  }
  .term-agent-item:hover, .term-agent-item:focus-visible {
    background: rgba(125,211,252,0.14); color: var(--accent); outline: none;
  }
...
(function () {
  ...
  // The single-pane rotation, in FAB-tap order. SPLIT is deliberately NOT in
  // it — entering and leaving split is the long-press menu's job alone.
  const CYCLE = ['term', 'open', 'view'];
  // The two panes that can occupy the right half of a split. TERM is pinned
  // to the left, so these are what the FAB swaps between while split.
  ...
  // the layout in BOTH directions, so a phone can be forced into split and a
  // landscape tablet forced out of it. Unset ⇒ the layout decides: 900px+
  // wide and landscape, judged by lib/split-layout.js against the tallest
  // height this width has had — NOT the live media query. The viewport meta
  // says interactive-widget=resizes-content, so the soft keyboard shrinks the
  // layout viewport, and '(orientation: landscape)' came true on a portrait
  // tablet every time the keyboard opened (B22).
  // It is also the ONLY way in or out of split: the FAB never leaves, so
  // splitEnabled() and currentView === 'split' stay in lockstep.
  const SPLIT_PREF_KEY = 'claude-hub:split:' + cfg.name;
  const splitLayout = (${makeSplitLayout.toString()})(900);

  // The "+" agent picker, shared with the Browse shell. Inlined via
  // .toString(), so it takes the agent list as an argument (V42).
  ${openAgentMenu.toString()}
  const TERM_AGENTS = ${JSON.stringify(TERM_AGENTS)};
  function layoutSplit() { return splitLayout(window.innerWidth, window.innerHeight); }
  let layoutSplitNow = layoutSplit();
  ...
  function splitEnabled() { return splitPref === null ? layoutSplitNow : splitPref; }
  ...
  function applyTermLabel(info, id, title) {
    ...
    info.tab.title = (title ? id + ' — ' + title : 'Tab ' + id) + suffix;
  }
  ...
  function buildTermTab(id, agent) {
    const tab = document.createElement('div');
    tab.className = 'term-tab';
    tab.dataset.id = id;
    tab.dataset.agent = agent;
    const badge = agent === 'claude' ? '' : '<span class="agent-tag">' + agent + '</span>';
    tab.innerHTML = '<span class="label"></span>' + badge + '<button type="button" class="close" title="Close tab" aria-label="Close tab">×</button>';
  ...
  function addTermTab(id, opts) {
    ...
    const tab = buildTermTab(id, agent);
    const iframe = buildTermIframe(id);
    const info = { tab, iframe, title: null, agent };
  ...
  async function createTermTab(agent) {
    try {
      const r = await fetch('/api/term-sessions/' + encodeURIComponent(cfg.name), {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ agent: agent || 'claude' }),
      });
      if (!r.ok) throw new Error('POST failed: ' + r.status);
      const body = await r.json();
      addTermTab(body.id, { activate: true, agent: body.agent });
  ...
  async function closeTermTab(id) {
    ...
    // Same agent as the tab just closed — closing your only codex tab must
    // not silently hand you claude.
    if (termTabs.size === 0) createTermTab(info.agent);
  }
  ...
  TERM_ADD_BTN.setAttribute('aria-haspopup', 'menu');
  TERM_ADD_BTN.setAttribute('aria-expanded', 'false');
  TERM_ADD_BTN.textContent = '+';
  TERM_ADD_BTN.addEventListener('click', () => {
    openAgentMenu(TERM_ADD_BTN, TERM_AGENTS, (agent) => createTermTab(agent));
  });
  ...
  async function initTermTabs() {
    ...
    for (const s of sessions) addTermTab(s.id, { title: s.title, agent: s.agent });
  ...
  function otherRight(v) { return v === 'open' ? 'view' : 'open'; }

  // Where the next FAB tap lands. Split is a mode, not a step in the loop:
  // while split, a tap swaps the right half (TERM is pinned left) and stays
  // split. Single-pane, it walks TERM → OPEN → VIEW — the third state is what
  // makes Browse reachable on a phone, where a split's right half, its only
  // other home, never appears.
  function nextView(cur) {
    if (cur === 'split') return otherRight(rightView);
  ...
  function swap() {
    if (currentView === 'split') {
      // Stay split, just move the other pane into the right half. The URL is
      // already ?view=split and stays that way.
      rightView = otherRight(rightView);
      applyView('split');
      return;
    }
  ...
  // preference is unset — once the user has chosen, the layout has no say.
  // Rotating INTO a split layout enters split, because the FAB can no longer
  // get you there; rotating out falls back to the right-half pane so nobody
  // is left staring at a half-width terminal on a phone. Every resize is
  // judged (the keyboard fires plenty) but only a changed verdict acts — and
  // the judge's whole point is that a keyboard never changes it (B22).
  window.addEventListener('resize', () => {
    const now = layoutSplit();
    if (now === layoutSplitNow) return;
    layoutSplitNow = now;
    if (splitEnabled()) go('split');
    else if (currentView === 'split') go(rightView);
    else syncFab();
  });
  ...
  splitBtn.addEventListener('click', () => {
    ...
    const on = !splitEnabled();
    setSplitPref(on);
    // This is the only way in or out of split. On: enter immediately, even on
    // an aspect ratio that would never have offered it. Off: drop to the pane
    // that was in the right half.
