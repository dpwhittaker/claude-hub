...
//
// xterm DOES empty it on blur (`_handleTextAreaBlur` sets `value = ''`), which
// is why the mirror is re-seeded on every `focusin` rather than only on first
// sight of the element — see that listener for what went wrong (B23).
//
...
function installAndroidInput(doc) {
  ...
  // Re-seed on every focus, not just first sight of the element.
  //
  // Two reasons, and the second is a bug the first version had (B23):
  //
  //   1. Bind the textarea as soon as it can receive input, so the mirror is
  //      seeded from an empty box. Without this, an `input` that arrives with
  //      no preceding keydown — voice input, a suggestion-strip tap — would be
  //      first sight of the element, and seeding the mirror from its
  //      already-updated value would swallow exactly that text.
  //   2. xterm's `_handleTextAreaBlur` does `this.textarea.value = ''`. Every
  //      blur therefore empties the box under us while the mirror still holds
  //      everything typed since the last trim — up to 512 chars. The next
  //      keystroke diffed "x" against that stale mirror and sent one DEL per
  //      stale char, erasing a line or two of whatever was already at the
  //      prompt. Leaving and returning to the terminal is an ordinary move
  //      here (the FAB cycles panes, Develop has tabs, the keyboard opens and
  //      closes), so this fired constantly.
  //
  // Whatever the box holds when focus lands is, by definition, neither ours to
  // send nor ours to delete: nothing can type into it while it is unfocused.
  // A composition cannot survive the focus change either, and an xterm-owned
  // key left in flight by a focus-moving keystroke must not swallow the first
  // input after we come back.
