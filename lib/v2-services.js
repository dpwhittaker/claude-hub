...
const SHOW_PROPS = ['Id', 'Description', 'ActiveState', 'SubState', 'MainPID', 'ExecMainStartTimestamp', 'UnitFileState', 'WorkingDirectory'];
...
// `ss -H -ltnp` → [{port, pid}] for listeners that name their process
// (only the caller's own processes do without root — which is every unit
// that runs as the hub's user).
function parseSsListeners(text) {
  const out = [];
  for (const line of String(text || '').split('\n')) {
    const port = /(?:^|\s)(?:\S+):(\d+)\s+\S+\s+users:\(/.exec(line);
    if (!port) continue;
    const re = /pid=(\d+)/g;
    let m;
    while ((m = re.exec(line))) out.push({ port: Number(port[1]), pid: Number(m[1]) });
  }
  return out;
}

// /proc/<pid>/cgroup → the systemd unit the process belongs to, or null.
function unitFromCgroup(text) {
  const m = /\/([^/\s]+\.service)\s*$/m.exec(String(text || ''));
  return m ? m[1] : null;
}

// Tailnet listeners → Map<localPort, https url> (the mount's target port).
function tailnetByPort(tailnet) {
  const map = new Map();
  for (const t of tailnet || []) {
    for (const mnt of t.mounts || []) {
      const m = /^https?:\/\/(?:localhost|127\.0\.0\.1|\[::1\])(?::(\d+))?(\/.*)?$/.exec(mnt.target || '');
      if (!m) continue;
      const port = Number(m[1] || (mnt.target.startsWith('https') ? 443 : 80));
      if (!map.has(port)) map.set(port, t.url + (mnt.path === '/' ? '/' : mnt.path));
    }
  }
  return map;
}
...
// Every sentinel under the root, at any depth (V99). `name` = the folder's
// basename (what `vite@<name>` instances are keyed by), `rel` = its path.
function readSentinels(projectsRoot) {
  return findSentinels(projectsRoot).map(({ rel, name, meta }) => ({ name, rel, meta }));
}

function defaultReadCgroup(pid) {
  try { return fs.readFileSync(`/proc/${pid}/cgroup`, 'utf8'); } catch { return ''; }
}
...
function makeServiceLister({ projectsRoot, hubDir, exec, unitDir = '/etc/systemd/system', user = os.userInfo().username, home = os.homedir(), readCgroup = defaultReadCgroup }) {
  ...
  async function discoverUnits() {
    ...
    for (const { name, rel, meta } of sentinels) {
  ...
  async function list() {
    ...
    // A unit no sentinel claims still belongs to the folder it runs in, when
    // that folder is under the root — that is what gives claude-hub.service
    // or a bare vite@<name> its branch chip (V101).
    const rootPrefix = path.resolve(projectsRoot) + '/';
    const cwdProject = (wd) => {
      const abs = typeof wd === 'string' && wd.startsWith('/') ? path.resolve(wd) : '';
      return abs.startsWith(rootPrefix) ? abs.slice(rootPrefix.length) : null;
    };
    const services = units.map((unit) => {
      ...
      return {
        ...
        project: info.project || cwdProject(s.WorkingDirectory),
        url: info.url || null,
      };
    });
    const tailnet = parseTailscaleServeStatus(await run('tailscale', ['serve', 'status']));
    // Which unit listens where — so a unit that only `tailscale serve`
    // publishes (opencode-web on :4096 → :8443) still opens its site (B33).
    const portsByUnit = new Map();
    for (const { port, pid } of parseSsListeners(await run('ss', ['-H', '-ltnp']))) {
      const unit = unitFromCgroup(readCgroup(pid));
      if (!unit) continue;
      if (!portsByUnit.has(unit)) portsByUnit.set(unit, new Set());
      portsByUnit.get(unit).add(port);
    }
    const byPort = tailnetByPort(tailnet);
    for (const svc of services) {
      svc.ports = [...(portsByUnit.get(svc.unit) || [])].sort((a, b) => a - b);
      svc.tailnetUrl = svc.ports.map((p) => byPort.get(p)).find(Boolean) || null;
      if (!svc.url && svc.tailnetUrl) svc.url = svc.tailnetUrl;
    }
...
module.exports = { makeServiceLister, unitIsUsers, parseSsListeners, unitFromCgroup, tailnetByPort, parseSystemctlShow, parseTailscaleServeStatus, parseListUnits, isUnitName, ACTIONS, SESSION_UNIT_RE };
