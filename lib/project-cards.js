...
// SPEC §V55 (revised). A worktree's category is its parent's. Its own README
// is whatever that branch happens to carry, so retagging the parent on main
// would leave every worktree card advertising the old tags until the branch
// merged — and the tag bar would keep minting chips for them (V73). The
// checkout's own README is only the fallback for a worktree whose parent is
// gone from the hub.
function inheritWorktreeTags(cards) {
  const byName = new Map(cards.map((c) => [c.name, c]));
  return cards.map((c) => {
    const parent = c.worktreeOf ? byName.get(c.worktreeOf) : null;
    if (!parent || parent === c) return c;
    const parentTags = Array.isArray(parent.tags) ? parent.tags : [];
    return { ...c, tags: [WORKTREE_TAG, ...parentTags.filter((t) => t !== WORKTREE_TAG)] };
  });
}

function buildProjectCards(rows) {
  return sortCards(inheritWorktreeTags(rows.map(buildCard)));
}
