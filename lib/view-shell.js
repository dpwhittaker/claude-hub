 ...
 *
 * Seven pure helpers are shared with the server by injecting their source
 * with `.toString()` rather than duplicating them — so they must stay
...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  /* Agent badge. Only non-default agents get one: a claude tab looks exactly
     as it always has, and a badge means "this one is something else". */
  .term-tab .agent-tag {
    font-size: 0.62rem; text-transform: uppercase; letter-spacing: 0.04em;
    padding: 1px 4px; border-radius: 3px; color: var(--muted);
    border: 1px solid var(--border); line-height: 1.3;
  }
  .term-tab.active .agent-tag { color: var(--fg); border-color: var(--accent); }
  /* The "+" menu. position:fixed — the tab strip scrolls horizontally, so a
     menu positioned inside it would drift off its own button. */
  .term-agent-menu {
    position: fixed; z-index: 60; min-width: 144px;
    background: var(--bg-1); border: 1px solid var(--border); border-radius: 6px;
    box-shadow: 0 8px 24px rgba(0,0,0,0.45); padding: 4px; display: flex;
    flex-direction: column;
  }
  .term-agent-item {
    border: none; background: transparent; color: var(--fg); text-align: left;
    font: inherit; font-size: 0.82rem; padding: 7px 10px; border-radius: 4px;
    cursor: pointer;
  }
...
async function closeTermTab(id) {
  ...
  // Spec: closing last tab spawns a fresh one — of the same agent, so
  // closing your only codex tab doesn't silently hand you claude.
