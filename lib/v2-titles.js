...
function makeTitleStore({ dir }) {
  ...
  // Cheap lookup for list views: one read, many gets. `fn(uuid)` → title,
  // `fn.rec(uuid)` → {title, at, source}.
  function lookup() {
    const map = readAll();
    const rec = (uuid) => (uuid && map[String(uuid).toLowerCase()]) || null;
    const fn = (uuid) => { const r = rec(uuid); return r ? r.title : null; };
    fn.rec = rec;
    return fn;
  }
