...
// Stream the most recent title record out of <encoded>/<uuid>.jsonl — either
// Claude's own `ai-title` or the `custom-title` a `/rename` writes; whichever
// was appended LAST wins, so a rename shows up on the tab (V90). We read the
// file in 64 KB chunks from the END backwards so a multi-megabyte session
// doesn't cost a full scan; the title is usually in the last few hundred KB.
// Returns null if no title is found or the file is missing.
    ...
    while (pos > 0) {
      ...
      for (let i = lines.length - 1; i >= 0; i--) {
        ...
        try {
          ...
          if (obj && obj.type === 'ai-title' && typeof obj.aiTitle === 'string') return obj.aiTitle;
          if (obj && obj.type === 'custom-title' && typeof obj.customTitle === 'string' && obj.customTitle.trim()) return obj.customTitle.trim();
        } catch {}
      }
    }
  } finally {
