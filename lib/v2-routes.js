...
const { readSessionTitle } = require('./term-sessions');
...
function makeV2Router(deps) {
  ...
  function withTitles(list) {
    ...
    return list.map((s) => {
      if (s.agent !== 'claude') return s;
      const abs = s.cwd ? path.join(projectsRoot, s.cwd) : projectsRoot;
      const title = s.title || auto(s.uuid) || readSessionTitle(abs, s.uuid) || null;
      return { ...s, title };
    });
  }
  ...
  async function runningKeys() {
    try { return new Set((await tmuxListSessions()).filter(Boolean)); } catch { return new Set(); }
  }
  ...
  async function handle(req, res, url) {
    ...
    try {
      ...
      if (p === '/api/v2/sessions') {
        if (m === 'GET') {
          const running = await runningKeys();
          const hub = sessions.list().map((s) => ({ ...s, running: running.has(s.termKey) }));
          const legacy = (listLegacySessions ? listLegacySessions() : []).map((s) => ({ ...s, title: null, running: running.has(s.termKey) }));
          return sendJson(res, 200, { sessions: withTitles([...hub, ...legacy]) }), true;
        }
      ...
      mm = /^\/api\/v2\/services\/([^/]+)\/(start|stop|restart|logs)$/.exec(p);
      if (mm) {
