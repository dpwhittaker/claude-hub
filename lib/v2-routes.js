...
const { readTranscriptTitle, readTranscriptLastAt } = require('./claude-transcript');
...
function makeV2Router(deps) {
  const {
    ...
    readProjectRoutes, readProjectProxyPrefix, tmuxListSessions,
    claudeBin, unitDir,
  } = deps;
  ...
  // = "most recent response": for a live claude session the registry's
  // status change (the last turn's end), for a stopped one the transcript's
  // last record, for codex/shell tmux's activity — never tmux activity for
  // claude, whose idle TUI repaints its footer every few minutes (B32).
  function withTitles(list, tmux, live) {
    const auto = titles.lookup();
    const branches = new Map();   // cwd → branch, once per list
    const branchFor = (cwd) => { if (!branches.has(cwd)) { try { branches.set(cwd, fsApi.branch(cwd).branch); } catch { branches.set(cwd, null); } } return branches.get(cwd); };
    return list.map((s) => {
      ...
      // Creation time only stands in until the session has produced a signal.
      const created = s.createdAt ? Date.parse(s.createdAt) || 0 : 0;
      let lastActive;
      if (s.agent !== 'claude') lastActive = (t ? t.activity : 0) || created;
      else if (reg) lastActive = Math.max(reg.statusUpdatedAt, reg.updatedAt) || created;
      else lastActive = readTranscriptLastAt(abs, uuid) || created;
      const out = { ...s, uuid, running: !!t, activity: reg ? reg.status : null, lastActive: lastActive || null, branch: branchFor(s.cwd || '') };
      if (s.agent === 'claude') {
        const candidates = [];
  ...
  // Persist the live id into the record so a reboot resumes the
  // conversation the user was actually in, not the one the tab began with.
  function followSessionId(s, sessionId) {
    try { sessions.update(s.id, { uuid: sessionId }); } catch {}
  }

  // Every session, annotated — the list view and the glasses shim share it.
  async function listSessions() {
    return withTitles(sessions.list(), await tmuxMap(), registry());
  }
  ...
  function previewUrlFor(rel) {
    const project = fsApi.projectOf(rel);   // the sentinel folder, any depth (V99)
    if (!project || project === rel) return null;
  ...
  async function handle(req, res, url) {
    ...
    try {
      ...
      if (p === '/api/v2/sessions') {
        if (m === 'GET') return sendJson(res, 200, { sessions: await listSessions() }), true;
      ...
      // ---- terminals: suspend = kill the tmux session; the record (and the
      // conversation) stay, the next attach starts it again (V95) ----
      mm = /^\/api\/v2\/term\/([^/]+)\/suspend$/.exec(p);
      if (mm) {
        if (m !== 'POST') return sendJson(res, 405, { error: 'method not allowed' }), true;
        const key = decodeURIComponent(mm[1]);
        if (!/^[A-Za-z0-9][A-Za-z0-9._-]{0,127}$/.test(key)) return sendJson(res, 400, { error: 'bad terminal key' }), true;
        const known = sessions.list().some((x) => x.termKey === key);
        if (!known) return sendJson(res, 404, { error: 'unknown terminal' }), true;
        try { await execFileP('tmux', ['kill-session', '-t', '=' + key], { timeout: 5000 }); }
        catch (e) { return sendJson(res, 404, { error: 'not running: ' + String(e.stderr || e.message).trim().slice(0, 120) }), true; }
        return sendJson(res, 200, { key, suspended: true }), true;
      }
      ...
      if (p === '/api/v2/services') {
        ...
        for (const svc of out.services) {
          if (!svc.project) { svc.branch = null; continue; }
          if (!branches.has(svc.project)) { try { branches.set(svc.project, fsApi.branch(svc.project).branch); } catch { branches.set(svc.project, null); } }
          svc.branch = branches.get(svc.project);
        }
      ...
      if (p === '/api/v2/fs/delete' && m === 'POST') return body(req, res, 4096, (b) => sendJson(res, 200, fsApi.remove(b.path))), true;
  ...
  return { handle, profiles, sessions, services, titles, fs: fsApi, listSessions, renderMarkdownPage };
}
