...
const { makeActivity } = require('./v2-activity');
const { readSessionTitle, encodeClaudeProjectDir } = require('./term-sessions');
...
function completeWithClaude({ before, after, lang, pathHint }, { claudeBin, model = COMPLETION_MODEL, timeoutMs = 60000 } = {}) {
  ...
  return new Promise((resolve, reject) => {
    const child = spawn(claudeBin, ['-p', '--output-format', 'text', '--model', model, '--no-session-persistence'], {
...
function makeV2Router(deps) {
  ...
  // Title precedence for a claude session (V90): an explicit hub title, then
  // the hook's latest auto title, then whatever the transcript carries
  // (`/rename` or Claude's own first naming), else nothing — the client falls
  // back to the folder name.
  // Also `activity` (busy/waiting/idle from the hooks, V92) and `lastActive`
  // = the latest of tmux activity, the transcript's mtime and the last hook
  // event — what "most recent response" means across claude/codex/shell.
  function withTitles(list, tmux) {
    const auto = titles.lookup();
    const act = activity.lookup();
    return list.map((s) => {
      const abs = s.cwd ? path.join(projectsRoot, s.cwd) : projectsRoot;
      const t = tmux.get(s.termKey);
      const a = s.agent === 'claude' ? act(s.uuid) : null;
      const times = [t ? t.activity : 0, s.agent === 'claude' ? transcriptMtime(abs, s.uuid) : 0, a ? a.at : 0, s.createdAt ? Date.parse(s.createdAt) || 0 : 0];
      const out = { ...s, running: !!t, activity: a ? a.state : null, lastActive: Math.max(...times) || null };
      if (s.agent === 'claude') out.title = s.title || auto(s.uuid) || readSessionTitle(abs, s.uuid) || null;
      return out;
    });
  }
    ...
    try {
      ...
      if (p === '/api/v2/sessions') {
        if (m === 'GET') {
          ...
          return sendJson(res, 200, { sessions: withTitles([...sessions.list(), ...legacy], tmux) }), true;
        }
      ...
      // ---- activity (the hooks' endpoint) ----
      if (p === '/api/v2/activity') {
        if (m !== 'POST') return sendJson(res, 405, { error: 'method not allowed' }), true;
        return body(req, res, 4096, (b) => sendJson(res, 200, activity.set(b.uuid, b.state))), true;
      }
  ...
  return { handle, profiles, sessions, services, titles, activity, fs: fsApi, renderMarkdownPage };
}
