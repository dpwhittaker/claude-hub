...
function completeWithClaude({ before, after, lang, pathHint }, { claudeBin, model = COMPLETION_MODEL, timeoutMs = 60000 } = {}) {
  ...
  return new Promise((resolve, reject) => {
    const child = spawn(claudeBin, ['-p', '--output-format', 'text', '--model', model, '--no-session-persistence'], {
      stdio: ['pipe', 'pipe', 'pipe'], env: { ...process.env, CLAUDECODE: '' },
    });
    let out = ''; let err = '';
...
function makeV2Router(deps) {
    ...
    try {
      ...
      if (p === '/api/v2/sessions') {
        if (m === 'GET') {
          ...
          const legacy = (listLegacySessions ? listLegacySessions() : []).map((s) => ({ ...s, running: running.has(s.termKey) }));
          return sendJson(res, 200, { sessions: [...hub, ...legacy] }), true;
        }
  ...
  return { handle, profiles, sessions, services, fs: fsApi, renderMarkdownPage };
}
