...
const { readTranscriptTitle, readTranscriptLastAt } = require('./term-sessions');
...
function makeV2Router(deps) {
  ...
  function withTitles(list, tmux, live) {
    ...
    return list.map((s) => {
      ...
      let lastActive;
      if (s.agent !== 'claude') lastActive = Math.max(t ? t.activity : 0, created);
      else if (reg) lastActive = Math.max(reg.statusUpdatedAt, reg.updatedAt, created);
      else lastActive = Math.max(readTranscriptLastAt(abs, uuid), created);
  ...
  async function handle(req, res, url) {
    ...
    try {
      ...
      if (mm) {
        ...
        const known = sessions.list().some((x) => x.termKey === key) || (listLegacySessions ? listLegacySessions() : []).some((x) => x.termKey === key);
