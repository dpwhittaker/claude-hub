...
| T77 | x | PWA shell: home link in the term tab strip; FAB gains a third single-pane state (VIEW = the Browse shell) so the cycle is always TERM→OPEN→VIEW; long-press opens a menu (target-aware Refresh + sticky per-device/per-project Split toggle) instead of refreshing outright | V57,V58,V59,V60 |
