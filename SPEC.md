...
| T78 | x | `lib/android-input.js` — Android-gated capture-phase input shim replacing xterm's lossy IME path (synchronous prefix-diff, codepoint-accurate DEL, keypress/focusin guards, tail-trim); de-thrash `lib/keyboard-fit.js` (dedupe no-op vv resizes, rAF-coalesce); wire `ANDROID_INPUT_INJECT` into the ttyd proxyRes blob; 21+5 tests incl. V42 round-trip | V61,V62,B17 |
