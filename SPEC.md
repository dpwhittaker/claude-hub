...
- V40: develop-pane terminal iframe ! translate touch-drag → synthetic `wheel` events on iframe document so xterm scrolls under finger drag. attach on iframe `load` (same-origin via proxy). QUANTIZED: accumulate drag px, emit 1 wheel per 2.5-text-line step (`step = 2.5 * viewportHeight/term.rows`, fallback 40px), deltaY = ±step. rationale: tmux (mouse on) puts outer xterm in alt buffer w/ zero local scrollback → every wheel = 1 mouse tick to tmux regardless of deltaY; per-touchmove dispatch @60Hz ≈ 60 ticks/s overshoot. tick lands in inner app (Claude Code transcript scroll — it requests mouse tracking, tmux default WheelUpPane passthrough MUST stay, see B14) or tmux copy-mode (5 lines/tick) for non-mouse apps. half-step → finger↔content ~2:1 (1:1 @ 5-line step shipped first, too slow to swipe back a long transcript on a phone). preventDefault every single-touch move (⊥ rubber-band). ⊥ stuck/overshoot terminal scroll on mobile/tablet.
